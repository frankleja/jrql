...
public class TemplateVariant implements TemplateContainer {
	...
	/**
	 * Returns the name of the template this variant belongs to.
	 */
	...
	/**
	 * Returns the template code for this template variant.
	 * 
	 * @see #getDetailsNode()
	 */
	...
	/**
	 * Change the template code for this template variant appending the given codeLines.
	 * <p>
	 * Attention: Make sure the element names used in given template code matches the element of the content class! This method did not
	 * check and complain anything.
	 */
	public void appendTemplateCode(String codeLines) throws RQLException {
		setTemplateCode(getTemplateCode() + "\n" + codeLines);
	}

	/**
	 * Change the template code for this template variant appending a HTML comment with the usage of the given element.
	 * 
	 ...
	 */
	public void appendTemplateCodeAddComment(TemplateElement referencedElement, boolean surroundCommentWithIoRangeReddotMode)
			throws RQLException {
		// check if element is in template
		if (!getTemplate().contains(referencedElement)) {
			throw new ElementNotFoundException("The template element with name " + referencedElement.getName()
					+ " could not be found within template " + getTemplateName()
					+ ". You cannot add this template element to the template code therefore. Please correct your request.");
		}
		// valid, add a comment
		String line = StringHelper.buildHtmlComment(referencedElement.getTemplateCode());
		if (surroundCommentWithIoRangeReddotMode) {
			line = StringHelper.surroundIoRangeRedDotMode(line);
		}
		appendTemplateCode(line);
	}

	/**
	 * Change the template code for this template variant.
	 * <p>
	 * Attention: Make sure the element names used in given template code matches the element of the content class! This method did not
	 * check and complain anything.
	 */
	public void setTemplateCode(String templateCode) throws RQLException {

		/* 
		 V9 request
		<IODATA loginguid="9CC38BA1647E4872B0575E60395EA2DB" sessionkey="C8E221786F5E448981016939BDC94261">
		  <PROJECT>
		    <TEMPLATE>
		      <TEMPLATEVARIANT action="save" guid="09942C2A20B94ACAA117194BC2ECC0AE">
		&lt;html&gt;&lt;title&gt;hier der neue code&lt;/title&gt;&lt;html&gt;
		</TEMPLATEVARIANT>
		    </TEMPLATE>
		  </PROJECT>
		</IODATA> 
		 V9 response
		<IODATA>09942C2A20B94ACAA117194BC2ECC0AE
		</IODATA>
		 */
		...
		// call CMS
		String rqlRequest = "<IODATA loginguid='" + getLogonGuid() + "' sessionkey='" + getSessionKey() + "'>"
				+ "<PROJECT><TEMPLATE><TEMPLATEVARIANT action='save' guid='" + getTemplateVariantGuid() + "'>"
				+ StringHelper.escapeHTML(templateCode) + "</TEMPLATEVARIANT></TEMPLATE></PROJECT></IODATA>";
		callCmsWithoutParsing(rqlRequest);
		// reset cache
		templateCodeCache = templateCode;
	}
