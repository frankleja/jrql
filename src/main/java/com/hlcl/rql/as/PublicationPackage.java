...
public class PublicationPackage implements ProjectContainer {
	 ...
	 */
	public PublicationSetting addSetting(ProjectVariant newProjectVariant, LanguageVariant languageVariant,
			ProjectVariant copyProjectVariant) throws RQLException {
		/* 
		 V7.5 request
		...
		String rqlRequest = "<IODATA loginguid='" + getLogonGuid() + "' sessionkey='" + getSessionKey() + "'>" + "<PROJECT>"
				...
				+ newProjectVariant.getProjectVariantGuid() + "' languagevariantguid='" + languageVariant.getLanguageVariantGuid()
				+ "' copyguid='" + copySetting.getProjectVariantGuid() + copySetting.getLanguageVariantGuid() + "' />" + " </PROJECT>"
				+ "</IODATA>";
	 ...
	 */
	private PublicationSetting findPublicationSetting(ProjectVariant projectVariant, LanguageVariant languageVariant)
			throws RQLException {
	 ...
	 */
	public PublicationSetting findSettingByProjectVariantNameAndLanguageCode(String projectVariantName, String languageCode)
			throws RQLException {

		return getSettingFor(getProject().getProjectVariantByName(projectVariantName), getProject().getLanguageVariantByLanguage(
				languageCode));
	}
	 ...
	 * @return ExportSetting or null, if not exists
	 */
	public PublicationSetting findSettingByProjectVarianGuidAndRfcLanguageId(String projectVariantGuid, String rfcLanguageId)
			throws RQLException {

		return getSettingFor(getProject().getProjectVariantByGuid(projectVariantGuid), getProject().getLanguageVariantByRfcLanguageId(
				rfcLanguageId));
	}
	 ...
	 * @return ExportSetting or null, if not exists
	 */
	public PublicationSetting findSettingByProjectVariantNameAndRfcLanguageId(String projectVariantName, String rfcLanguageId)
			throws RQLException {

		return getSettingFor(getProject().getProjectVariantByName(projectVariantName), getProject().getLanguageVariantByRfcLanguageId(
				rfcLanguageId));
	}
	...
	private RQLNode getDetailsNode() throws RQLException {
		...
		*/
		...
		if (detailsNodeCache == null) {
			String rqlRequest = "<IODATA loginguid='" + getLogonGuid() + "' sessionkey='" + getSessionKey() + "'>" + "  <PROJECT>"
					+ "   <EXPORTPACKET action='loadpacket' guid='" + getPublicationPackageGuid() + "'/>" + "  </PROJECT>"
					+ "</IODATA>";
	...
	/**
	 * Liefert die GUID des PublicationFolders für die gegebenen folderGuid aus dem gegebenen node für das PublicationSetting.
	 * <p>
	 * Liefert null, falls für die folderGuid ins root Verzeichnis publiziert wird.
	 * <p>
	 * 
	 * @throws ElementNotFoundException
	 *             if folderGuidToFind not found within publicationSettingNode
	 */
	private String getPublicationFolderGuid(RQLNode publicationSettingNode, String folderGuidToFind) throws RQLException {
		...
		// signal not found
		throw new ElementNotFoundException("Folder GUID " + folderGuidToFind + " could not found in RQL response node with guid "
				+ publicationSettingNode.getAttribute("guid") + " project variant "
				+ publicationSettingNode.getAttribute("projectvariantname") + " language variant "
				+ publicationSettingNode.getAttribute("languagevariantname") + " in publication package " + getName());
	}
	...
	RQLNode getPublicationSettingNode(PublicationSetting publicationSetting) throws RQLException {
		...
		// should not happen
		throw new ElementNotFoundException("RQLNode for publication setting " + publicationSetting.getName()
				+ " in publication package " + this.getName() + " not found. Maybe cached values are not in synch.");
	}
	...
	/**
	 * Creates a publication setting object from the RQLNode. TODO Funktioniert nicht, falls der publication folder für published pages
	 * nicht direkt unter der root liegt.
	 * 
	 * @throws RQLException
	 */
	private PublicationSetting buildPublicationSetting(RQLNode node) throws RQLException {
		...
		// create object
		return new PublicationSetting(this, node.getAttribute("guid"), folder, node.getAttribute("projectvariantguid"), node
				.getAttribute("languagevariantguid"));
	}
	...
	public PublicationSetting getSettingFor(String projectVariantGuid, String languageVariantGuid) throws RQLException {
		...
		return getSettingFor(getProject().getProjectVariantByGuid(projectVariantGuid), getProject().getLanguageVariantByGuid(
				languageVariantGuid));
	}
	...
	/**
	 * Returns all settings matching the given project variants and language variants which are configured for publishing to the given
	 * publishingTarget which publishing goes into the given rootFolder (e.g. docRoot).
	 */
	public java.util.List<PublicationSetting> getSettings(java.util.List<ProjectVariant> projectVariants,
			java.util.List<LanguageVariant> languageVariants, PublishingTarget publishingTarget, PublicationFolder rootFolder) throws RQLException {
		java.util.List<PublicationSetting> result = new ArrayList<PublicationSetting>();

		// select settings
		for (PublicationSetting setting : getSettings(projectVariants, languageVariants)) {
			if (setting.hasPublishingTo(publishingTarget) && setting.isPublishedPagesStartsWith(rootFolder.getName())) {
				result.add(setting);
			}
		}
		return result;
	 ...
	 */
	public java.util.List<PublicationSetting> getSettings(java.util.List<ProjectVariant> projectVariants,
			java.util.List<LanguageVariant> languageVariants, PublishingTarget publishingTarget) throws RQLException {
		java.util.List<PublicationSetting> result = new ArrayList<PublicationSetting>();

		// select settings
		for (PublicationSetting setting : getSettings(projectVariants, languageVariants)) {
			if (setting.hasPublishingTo(publishingTarget)) {
				result.add(setting);
			}
		}
		return result;
	...
	/**
	 * Returns all settings matching the given project variants and language variants.
	 */
	public java.util.List<PublicationSetting> getSettings(java.util.List<ProjectVariant> projectVariants,
		...
		// select settings
		for (PublicationSetting setting : getPublicationSettings()) {
			if (projectVariants.contains(setting.getProjectVariant()) && languageVariants.contains(setting.getLanguageVariant())) {
				result.add(setting);
			}
		}
		return result;
	 ...
	 * 
	 * @deprecated Did not work in V7.5.0.33. Deliver same attributes within same segment tag what causes the xml parser to stop with
	 *             an error.
	 */
