...
/**
 * Diese Klasse beschreibt einen manuell erstellen Publizierungsauftrag, einen laufenden und einen vordefinierten.
 * TODO should use the State pattern and implement possible transitions between. 
 * @author LEJAFR
 */
public class PublishingJob implements ProjectContainer {
	 ...
	 * @throws RQLException
	 */
	public PublishingJob(Project project, Page startPageOrNull, String name, String asyncQueueGuid, String category, String status, String jobGuid,
			String startedByUserName) throws RQLException {
		super();
	...
	public void addToPublish(ProjectVariant projectVariant, LanguageVariant languageVariant) throws RQLException {
		...
		if (publicationPackage != null && !publicationPackage.containsSettingFor(projectVariant, languageVariant)) {
			throw new InvalidPublishingRequestException("Page with ID " + startPage.getPageId() + " could not published for project variant named "
					+ projectVariant.getName() + " and language variant named " + languageVariant.getName()
					+ ", because the publication package named " + publicationPackage.getName()
	...
	/**
	 * Dieser Generierungsauftrag wird auch die gegebenen Projektvariante in der gegebenen Sprachvariante mit languageVariantGuid generieren.
	 * <p>
	 ...
	 */
	public void addToPublish(String projectVariantGuid, String languageVariantGuid, boolean checkThatLanguageVariantIsAccessible) throws RQLException {
	 ...
	 */
	public void addToPublishAllCombinations(String projectVariantGuids, String separator, String languageVariantGuids) throws RQLException {
	 ...
	 * Dieser Generierungsauftrag wird alle Kombinationen der gegebenen Projekt- und Sprachvarianten generieren.
	 */
	public void addToPublishAllCombinations(String projectVariantGuids, String separator, java.util.List<LanguageVariant> languageVariants) throws RQLException {
		// for all language variants publish all project variants
	 ...
	 * Dieser Generierungsauftrag wird alle Kombinationen der gegebenen Projekt- und Sprachvarianten generieren.
	 */
	public void addToPublishAllCombinations(java.util.List<ProjectVariant> projectVariants, java.util.List<LanguageVariant> languageVariants)
			throws RQLException {
	...
	public void cancel() throws RQLException {
		...
		// call CMS
		String rqlRequest = "<IODATA loginguid='" + getLogonGuid() + "'>" + " <PROJECT projectguid='" + getProjectGuid() + "' sessionkey='"
				+ getSessionKey() + "'>" + " <EXPORT action='cancel' guid='" + getAsyncQueueGuid() + "'/>" + "</PROJECT>" + "</IODATA>";
		// ignore result
		callCms(rqlRequest);
	}
	...
	public void start() throws RQLException {
		...
		// build request
		String rqlRequest = "<IODATA loginguid='" + getLogonGuid() + "' sessionkey='" + getSessionKey() + "'>" + "<PROJECT guid='" + getProjectGuid()
				+ "'>" + "   <PAGE guid='" + startPage.getPageGuid() + "'>" + "    <EXPORTJOB action='save' generatenextpages='"
				+ (withFollowingPages ? "1" : "0") + "' generaterelativepages='" + (withRelatedPages ? "1" : "0") + "' email='"
				+ (mailReceiver != null ? mailReceiver.getUserGuid() : "") + "'>" + "     <LANGUAGEVARIANTS action='checkassigning'>";
	...
	private RQLNode getDetailsNode() throws RQLException {
		...
		if (detailsNode == null) {
			...
			+ "<PROJECT projectguid='" + getProjectGuid() + "' >"
					+ "   <EXPORTJOB action='load' guid='" + getPublishingJobGuid() + "'/>" + "</PROJECT>" + "</IODATA>";
	...
	private void save(String attributeName, String attributeValue) throws RQLException {
		...
		String rqlRequest = "<IODATA loginguid='" + getLogonGuid() + "' sessionkey='" + getSessionKey() + "'><PROJECT>"
				+ "<EXPORTJOB action='save' guid='" + getPublishingJobGuid() + "' " + attributeName + "='" + attributeValue + "' />" + "</PROJECT></IODATA>";
